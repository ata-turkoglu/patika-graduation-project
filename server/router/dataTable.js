...
router.post("/addnewrow", async(req,res)=>{
  ...
  try {
    let result = await client.query(`INSERT INTO ${tableName} (${keys.join(", ")}) VALUES(${queryKeys}) RETURNING *`,values)
    res.status(200).json(result)
  } catch (error) {
    console.error(error)
    res.status(500).json({error, message:`could not add row to ${tableName}`})
  }
})
...
router.delete("/deleterow", async(req,res)=>{
  ...
  try {
    ...
    console.error(error)
    res.status(500).json({error, message:`could not delete row from ${tableName}`})
  }
})
...
router.patch("/updaterow",async(req,res)=>{
  ...
  try {
    let result = await client.query(`UPDATE ${tableName} SET ${query} WHERE id=$1 RETURNING *`, values)
    res.status(200).json(result)
  } catch (error) {
    console.error(error)
    res.status(500).json({error, message:`could not update row from ${tableName}`})
  }  
})
...
router.post("/addnewcolumn",async(req,res)=>{
  ...
  try {
    ...
    console.error(error)
    res.status(500).json({error, message:`could not add column to ${tableName}`})
  }
})
